...
# -----------------------------
    ...
    for ev in eventos:
        ...
        st.markdown(
    f"""
<div style="background:{cor_base}; color:{cor_fonte}; padding:14px 16px; border-radius:15px; margin-bottom:12px;
            opacity:{opac}; text-decoration:{decor};
            border:{borda_4_lados}; border-left:{barra_esquerda};">

    <h3 style="margin:0 0 8px 0; font-size:19px; line-height:1.3;">
    ...
    <div style="display:flex; flex-wrap:wrap; gap:20px; font-size:14px; line-height:1.4;">

        <div style="flex:1; min-width:220px;">
            <b>📅 Data:</b> {d_dt.strftime('%d/%m/%Y')}<br>
            <b>⏰ Horário:</b> {formatar_hora(ev['hora_inicio'])} às {formatar_hora(ev['hora_fim'])}<br>
            <b>📍 Local:</b> {ev['local']}<br>
            <b>🏠 Endereço:</b> {ev['endereco']}
        </div>
